...
def run_schema_migrations_after_models(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        if "face_photo_url" not in cols:
            if dialect == "postgresql":
                conn.execute(
                    text(
                        "ALTER TABLE cards ADD COLUMN IF NOT EXISTS face_photo_url VARCHAR(500)"
                    )
                )
            else:
                conn.execute(text("ALTER TABLE cards ADD COLUMN face_photo_url VARCHAR(500)"))
        ...
        if "animation_scenario_id" not in cols:
            if dialect == "postgresql":
                conn.execute(
                    text(
                        "ALTER TABLE cards ADD COLUMN IF NOT EXISTS "
                        "animation_scenario_id VARCHAR(100)"
                    )
                )
            else:
                conn.execute(
