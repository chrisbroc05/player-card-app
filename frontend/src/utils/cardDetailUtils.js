...
export function normalizeCardForDisplay(card) {
  ...
  return {
    ...
    rarity_template: card.rarity_template ?? card.rarityTemplate ?? 1,
    template_name: card.template_name ?? card.templateName ?? "",
    rarity_display_name: card.rarity_display_name ?? card.rarityDisplayName ?? "",
    ...
    face_photo_url: card.face_photo_url ?? card.facePhotoUrl ?? null,
    animation_motion: card.animation_motion ?? card.animationMotion ?? null,
    animation_scenario_id: card.animation_scenario_id ?? card.animationScenarioId ?? null,
