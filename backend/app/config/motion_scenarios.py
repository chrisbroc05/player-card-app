...
_FIELDING_GROUND_SCENARIOS: list[MotionScenario] = [
    ...
    _s(
        ...
        "Cinematic slow motion sports video. The second baseman is at second base receiving the incoming throw to turn a double play. The second baseman approaches second base from the first base side, catching the single baseball while their back foot touches the bag. As they catch, they simultaneously execute a pivot — pushing off the bag with the back foot and hopping to the right side (toward the outfield) to avoid the incoming runner. The pivot involves a small jump and body rotation, the second baseman landing with their body now facing first base. Throwing arm loads from the glove to cocked position during the pivot, front foot lands firmly toward first base, hips and shoulders rotate toward first, arm drives through at three-quarter slot, single baseball released on a tight line toward first base, arm following through toward first, body finishing balanced facing first base. The entire catch-pivot-throw is one explosive athletic sequence lasting about 1 second — fast and efficient over the next 5 seconds. Only one baseball shown.",
    ),
    _s(
        ...
        "Cinematic slow motion sports video. The shortstop is at second base receiving the incoming throw to turn a double play. The shortstop approaches second base from the third base side and catches the single baseball while dragging their foot across the bag or touching the corner. Unlike the second baseman pivot, the shortstop uses momentum — they are moving from the third base side toward first base which helps redirect their throw toward first. After catching, they plant the back foot firmly and step through toward first base with the front foot, body momentum naturally flowing toward first. Throwing arm loads quickly during the catch, coming to cocked position, hips and shoulders rotating toward first, arm driving through at three-quarter slot, single baseball released on a line to first, wrist snapping, arm following through toward first base. The shortstop may jump to avoid the runner — a small jump off the bag while throwing, landing after the throw releases. Explosive athletic sequence over the next 5 seconds. Only one baseball shown.",
    ),
]
